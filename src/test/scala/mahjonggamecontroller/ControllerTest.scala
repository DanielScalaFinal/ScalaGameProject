package test.scala.mahjonggamecontroller
...
import main.scala.mahjonggamemodel.Scores
...
class FakeView extends View {
  var started = false
  var stopped = false
  var startedController:MahjongController = null
  var stoppedController:MahjongController = null
  ...
  override def startView(controller:MahjongController) { startedController = controller; started = true; }
  override def stopView(controller:MahjongController) { stoppedController = controller; stopped = true; }
...
class ControllerSpec extends SpecificationWithJUnit {
  ...
  def createTestSetup : (FakeGame, FakeView, MahjongController) = {
    val fakeGame = new FakeGame(scoreFileName)
    val fakeView = new FakeView
    val controller = new MahjongController(fakeGame) { override def closeApplication {} }
